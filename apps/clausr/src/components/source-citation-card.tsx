...
>(function PdfPreviewPage({ fileUrl, filename, highlightChunk, onDisplaySizeChange }, ref) {
  ...
  if (highlightChunk.wordBoxes && refWidth > 0 && refHeight > 0 && dispSize) {
    const sx = dispSize.w / refWidth;
    const sy = dispSize.h / refHeight;
  ...
  const src = pageUrl(fileUrl, highlightChunk.pageNumber ?? 1);

  function calcDisplaySize(naturalW: number, naturalH: number) {
    const scale = Math.min(containerW / naturalW, containerH / naturalH);
    return { w: naturalW * scale, h: naturalH * scale, offsetX: (containerW - naturalW * scale) / 2, offsetY: (containerH - naturalH * scale) / 2 };
  }
  ...
  return (
    <div style={{ flexShrink: 0 }}>
      ...
      >
        {loadError ? (
          <span className="flex items-center justify-center h-full" style={{ color: "var(--color-text-muted)", fontSize: 11 }}>Page unavailable</span>
        ) : (
          <img
            ref={imgRef}
            src={src}
            alt={`Page ${highlightChunk.pageNumber}`}
            style={{ width: "100%", height: "100%", objectFit: "contain", display: "block" }}
            onLoad={(e) => {
              const img = e.currentTarget;
              const w = img.naturalWidth;
              const h = img.naturalHeight;
              const sz = calcDisplaySize(w, h);
              setDispSize(sz);
              onDisplaySizeChange(sz);
            }}
            onError={() => setLoadError(true)}
          />
        )}
        {highlightRects.length > 0 && !loadError && dispSize && (
          <div className="absolute inset-0 pointer-events-none" style={{ left: 0, top: 0, width: containerW, height: containerH }}>
            {highlightRects.map((rect, i) => (
              <div
                key={i}
                className="absolute"
                style={{
                  left: rect.x + (containerW - dispSize.w) / 2,
                  top: rect.y + (containerH - dispSize.h) / 2,
                  width: rect.width,
                  height: rect.height,
