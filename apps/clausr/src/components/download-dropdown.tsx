...
import { generateDocx } from "@clausr/engine/client";
...
export function DownloadDropdown({ response, skillName }: DownloadDropdownProps) {
  ...
  useEffect(() => {
    if (open && buttonRef.current) {
      ...
      const top = spaceBelow >= menuH ? rect.bottom + gap : rect.top - gap - menuH;
      const rightSpace = window.innerWidth - rect.right;
      const right = Math.min(Math.max(rightSpace, 8), window.innerWidth - w - 8);
      setMenuStyle({
        position: "fixed",
        top,
        right,
  ...
  const menu = open && (
    ...
    >
      ...
      <DownloadOption
        ...
        label="PDF (.pdf)"
        hint="coming soon"
        onClick={() => { closeMenu(); alert("PDF export coming soon."); }}
        disabled={false}
      />
      <div style={{ height: 1, background: "var(--color-border-default)" }} />
      <a
        ...
        style={{
          ...
          color: "var(--color-text-body)", fontSize: 12,
        }}
      ...
      >
        ...
        <span style={{ marginLeft: "auto", fontSize: 10, color: "var(--color-text-muted)" }}>learn more</span>
      </a>
    </div>
  );
