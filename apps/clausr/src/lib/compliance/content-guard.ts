...
// Used by the chat route before forwarding user input to the LLM.
//
...
const A2_PATTERNS: Record<string, RegExp[]> = {
  ...
  "discrimination-disease": [/(aids|hiv|covid|leprosy)\s+patients?\s+(deserve|should\s+be\s+(isolated|quarantined))/i, /(carrier|patient)\s+of\s+(disease|virus)\s+(should|must)\s+be\s+(isolated|removed)/i, /(艾滋病|新冠|麻风)\s+(患者|病人)\s+(活该|应被隔离)/i],
...
const A4_PATTERNS: Record<string, RegExp[]> = {
  ...
  "privacy-violation": [/(dox|doxx|personal\s+(info|data|detail))\s+(leak|expose|publish|share)/i, /(home\s+)?address\s+of\s+[^\s]+\s+(is|:) \d+/i, /(phone|social\s+security|credit\s+card|passport)\s+(number|no)[\s:]+\d/i, /(人肉|开盒)\s+(搜索|曝光|泄露)/i, /(个人信息|隐私)\s+(泄露|曝光|发布)/i],
